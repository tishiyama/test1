...
function parseJwt(t){
  const p = t.split('.');
  if(p.length !== 3) throw new Error('bad jwt');
  return JSON.parse(b64urlDecode(p[1]));
}
...
(function showSignin(){
  ...
  try{
    ...
    status.textContent = `Signed in as ${payload.email || payload["cognito:username"] || payload.sub}`;
